// TODO: None of this is related to aws

import hash from '../../util/hash';
import { normalize } from '../../../boot/util/normalize';
...
export class TfEntity {
  ...
  static terraformEncode = (val: Json): string => {
    ...
    if (isForm(val, String))  return val[hasHead]('| ') ? val.slice('| '.length) : `"${slashEscape(val, '"\n')}"`;
    if (isForm(val, Number))  return `${val.toString(10)}`;
    if (isForm(val, Boolean)) return val ? 'true' : 'false';
    
    if (isForm(val, Array)) {
      const vals = val[map](v => this.terraformEncode(v));
      if (vals.some(v => isForm(v, String) && v[hasHead]('| '))) process.exit(0);
    ...
    if (isForm(val, Object)) {
      ...
      // Using "$" with strings causes "raw use"; the value won't be quoted (enabled complex tf)
      // Using "$" with object causes value to be a "nested block" instead of an "inline map"
    ...
    throw Error('unexpected val')[mod]({ form: getFormName(val), val });
  ...
  tfRef(props: string | string[] = []): string {
    ...
    if (!isForm(props, Array)) props = [ props ];
